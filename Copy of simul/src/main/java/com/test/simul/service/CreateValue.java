		...
		for (int k = 0; k < metricValues.getConfig_value(); k++) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if (plugin.equals("df")) {
					...
					if (type_instance.equals("free") && (df_freeCount < metricValues.getDf_partitions().size())) {
						// partition = metricValues.getDf_partitions().get();
						value[0] = metricValues.getDf_free().get(df_freeCount);

						df_freeCount++;
					} else if (type_instance.equals("used") && (df_usedCount < metricValues.getDf_partitions().size())) {
						partition = metricValues.getDf_partitions().get(df_usedCount);
					...
					simMetricVo.setPlugin_instance(partition);
